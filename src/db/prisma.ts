      ...
      async $allOperations({ args, query }) {
        ...
        for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
          try {
            ...
            console.warn(
              `Database unreachable, retrying in ${wait}ms (attempt ${attempt + 1}/${MAX_ATTEMPTS})`,
            );
            await sleep(wait);
          }
        }
 ...
 * immediately.
 */
export const prisma = (globalThis.prismaGlobal as ReturnType<typeof createClient> | undefined) ?? createClient();
