 ...
 */
exports.paymentsRouter.post("/subscribe", auth_1.requireAuth, async (req, res) => {
    ...
    const plan = (0, plans_1.getPlan)(plans_1.CURRENT_PLAN_CODE);
    if (!plan?.razorpayPlanId) {
        console.error(`Plan "${plans_1.CURRENT_PLAN_CODE}" has no razorpayPlanId configured`);
        return res.status(503).json({
            ok: false,
            error: { code: "PAYMENTS_UNAVAILABLE", message: "Payments aren't set up yet." },
        });
...
exports.paymentsRouter.post("/webhook", async (req, res) => {
    ...
    if (!(0, razorpay_1.isValidWebhookSignature)(rawBody, signature)) {
    ...
    try {
        ...
        if (eventType === "subscription.charged") {
            const planCode = subscriptionEntity?.notes?.planCode ?? plans_1.CURRENT_PLAN_CODE;
            const plan = (0, plans_1.getPlan)(planCode);
            if (!plan) {
            ...
            await prisma_1.prisma.$transaction([
                ...
                prisma_1.prisma.subscription.updateMany({
